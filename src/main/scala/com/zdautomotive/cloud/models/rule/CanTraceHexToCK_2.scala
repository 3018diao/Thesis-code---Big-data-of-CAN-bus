...
import com.zdautomotive.cloud.utils.{CANTools, CKDataSourceSingleton, DBCTools, DataSourceSingleton}
import org.apache.spark.sql.{DataFrame, Row}

import scala.collection.mutable.ListBuffer

class CanTraceHexToCK_2 extends BasicModel {
  ...
  override def getTraceData(): DataFrame = {
    //    val frame = CANTools.getHexTrace("file:////root/spark-3.1.1-bin-hadoop3.2/data/canTrace", sc, spark)
  ...
  override def saveResult(resultDF: DataFrame, dbcDF: DataFrame): Unit = {
    ...
    resultDF.foreachPartition((partition: Iterator[Row]) => {
      println("**********************")
      val dsPool = CKDataSourceSingleton.getDataSourceInstance
      ...
      partition.foreach {
        row =>
          ...
          //          psmt.setTimestamp(1, new Timestamp(ts))
          psmt.setObject(1, new Timestamp(ts))
...
object CanTraceHexToCK_2 {
  def main(args: Array[String]): Unit = {
    val model = new CanTraceHexToCK_2
    model.executeModel()
  }
}
