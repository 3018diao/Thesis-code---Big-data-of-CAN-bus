...
import com.datastax.driver.core.{BatchStatement, BoundStatement, Cluster, HostDistance, PoolingOptions, PreparedStatement}
import com.zdautomotive.cloud.models.BasicModel
import com.zdautomotive.cloud.utils.{CANTools, CassandraDataSourceSingleton, DBCTools, MYSQLDataSourceSingleton}
...
class CanTraceHexToCassandra_1 extends BasicModel {
  ...
  override def parseTrace(dbcDF: DataFrame, traceDF: DataFrame): DataFrame = {
    ...
    CANTools.parseRawCan(dbcDF, traceDF)
    //    frame.show(100,false)
...
object CanTraceHexToCassandra_1 {
  def main(args: Array[String]): Unit = {
    val start_time = new Date().getTime
    val model = new CanTraceHexToCassandra_1
    model.executeModel()
